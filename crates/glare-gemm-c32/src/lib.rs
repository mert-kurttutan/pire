...
use glare_base::{
    ap_size, bp_size, get_cache_params, has_f32_compute, Array, ArrayMut, GemmCache, GlarePar, HWModel,
    RUNTIME_HW_CONFIG,
};
...
fn get_mcnckc() -> (usize, usize, usize) {
    ...
    let (mc, nc, kc) = match (*RUNTIME_HW_CONFIG).hw_model {
        ...
        HWModel::Haswell => (2400, 384, 192),
        _ => get_cache_params(),
    };
    (mc, nc, kc)
}

pub(crate) unsafe fn glare_cgemm_generic<F: MyFn>(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
...
pub unsafe fn glare_sgemv(
    ...
    glare_cgemm(m, 1, n, alpha, a, a_rs, a_cs, x, 1, incx, beta, y, 1, incy)
}
pub unsafe fn glare_sdot(
    ...
    glare_cgemm(1, 1, n, alpha, x, incx, 1, y, incy, 1, beta, res, 1, 1)
}
                ...
                for k in k_dims.iter() {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glare_cgemm_generic(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_c32(
                                    m,
