...
#[inline(always)]
unsafe fn v_loadu_n(mem_addr: *const TC, n: usize) -> __m128d {
    let mut a_arr = [0_f64; VS];
    copy_nonoverlapping(mem_addr, a_arr.as_mut_ptr(), n);
    _mm_loadu_pd(a_arr.as_ptr())
}

unsafe fn fmadd_pd(a: __m128d, b: __m128d, c: __m128d) -> __m128d {
    let x = _mm_mul_pd(a, b);
    _mm_add_pd(x, c)
}
...
pub(crate) unsafe fn acc_store(
    ...
    let mut cv = cv;
    cv = fmadd_pd(_mm_loadu_pd(a.add(lda)), xt1, cv);
    cv = fmadd_pd(_mm_loadu_pd(a.add(lda * 2)), xt2, cv);
    cv = fmadd_pd(_mm_loadu_pd(a.add(lda * 3)), xt3, cv);
    _mm_storeu_pd(c, cv);
}
...
pub(crate) unsafe fn axpy_v_inner<const BETA: usize>(
    ...
    while mi < m_lane4 {
        seq!(i in 0..4 {
            let c~i = if BETA == 1 {
                fmadd_pd(_mm_loadu_pd(a.add(VS*i)), xt0, _mm_loadu_pd(y.add(VS*i)))
            } else if BETA == 0 {
                _mm_mul_pd(_mm_loadu_pd(a.add(VS*i)), xt0)
            } else {
                let cx~i = _mm_mul_pd(_mm_loadu_pd(y.add(VS*i)), beta_v);
                fmadd_pd(_mm_loadu_pd(a.add(VS*i)), xt0, cx~i)
            };
    ...
    while mi < m_lane {
        let c0 = if BETA == 1 {
            fmadd_pd(_mm_loadu_pd(a), xt0, _mm_loadu_pd(y))
        } else if BETA == 0 {
            _mm_mul_pd(_mm_loadu_pd(a), xt0)
        } else {
            let cx0 = _mm_mul_pd(_mm_loadu_pd(y), beta_v);
            fmadd_pd(_mm_loadu_pd(a), xt0, cx0)
        };
...
pub(crate) unsafe fn axpy_v(
    ...
    let mut a_cur = a;
    let mut x_cur = x;
    let mut xtv_arr = [_mm_setzero_pd(); K_UNROLL];
    while ni < n_lane {
        let mut xt_arr = [0.0; K_UNROLL];
        for i in 0..K_UNROLL {
            xt_arr[i] = *alpha * *x_cur.add(i * incx);
            xtv_arr[i] = _mm_set1_pd(xt_arr[i]);
        }
    ...
    while ni < n {
        let xt = *x_cur * *alpha;
        let xt_ptr = &xt as *const f64;
        let xt0 = _mm_set1_pd(*xt_ptr);
        if beta == 1.0 {
...
#[target_feature(enable = "sse")]
pub(crate) unsafe fn acc_vec(x: __m128d) -> TC {
    let mut acc_arr = [0.0; VS];
    _mm_storeu_pd(acc_arr.as_mut_ptr(), x);
    acc_arr[0] + acc_arr[1]
}
...
pub(crate) unsafe fn axpy_d(
    ...
    while i < m3 {
        ...
        let mut x_cur = x;
        let mut acc_arr = [_mm_setzero_pd(); 4 * 3];
        let mut p = 0;
        while p < n_iter_unroll_vec {
            seq!(q in 0..3 {
                acc_arr[q*4] = fmadd_pd(_mm_loadu_pd(a_cur.add(lda*q)), _mm_loadu_pd(x_cur), acc_arr[q*4]);
                acc_arr[q*4+1] = fmadd_pd(_mm_loadu_pd(a_cur.add(lda*q+VS)), _mm_loadu_pd(x_cur.add(VS)), acc_arr[q*4+1]);
                acc_arr[q*4+2] = fmadd_pd(_mm_loadu_pd(a_cur.add(lda*q+VS*2)), _mm_loadu_pd(x_cur.add(VS*2)), acc_arr[q*4+2]);
                acc_arr[q*4+3] = fmadd_pd(_mm_loadu_pd(a_cur.add(lda*q+VS*3)), _mm_loadu_pd(x_cur.add(VS*3)), acc_arr[q*4+3]);
            });
        ...
        seq!(q in 0..3 {
         ...
         acc_arr[q*4] = _mm_add_pd(acc_arr[q*4], acc_arr[q*4+1]);
         acc_arr[q*4] = _mm_add_pd(acc_arr[q*4], acc_arr[q*4+2]);
         acc_arr[q*4] = _mm_add_pd(acc_arr[q*4], acc_arr[q*4+3]);
        });
    ...
    while i < m {
        let mut a_cur = a_cur0;
        let mut x_cur = x;
        let mut acc_arr = [_mm_setzero_pd(); 4];
        let mut p = 0;
        while p < n_iter_unroll_vec {
            seq!(q in 0..1 {
                acc_arr[q*4] = fmadd_pd(_mm_loadu_pd(a_cur.add(lda*q)), _mm_loadu_pd(x_cur), acc_arr[q*4]);
