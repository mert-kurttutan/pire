...
#[inline(always)]
unsafe fn scale(v: float32x4_t, alpha: TC) -> float32x4_t {
    vmulq_n_f32(v, alpha)
}
...
impl AxpyKernel {
    ...
    pub(crate) unsafe fn accumulate(&mut self, i: usize, lda: usize, a_cur: *const TC, x: TC) {
        ...
        self.acc[0] = vfmaq_n_f32(self.acc[0], a_v_0, x);
        self.acc[1] = vfmaq_n_f32(self.acc[1], a_v_1, x);
    }
...
pub(crate) unsafe fn axpy_d(
    ...
    while i < m_unroll {
        ...
        seq!(p in 0..2 {
            vst1q_f32(acc_arr.as_mut_ptr(), acc_v[p]);
            acc_s[p] += acc_arr[0] + acc_arr[1] + acc_arr[2] + acc_arr[3];
            acc_s[p] *= alpha_t;
        });
    ...
    while i < m {
        ...
        while j < n_vec {
            ...
            acc_v[0] = vfmaq_f32(acc_v[0], a_p, x_v_0);
        ...
        vst1q_f32(acc_arr.as_mut_ptr(), acc_v[0]);
        acc_s += acc_arr[0] + acc_arr[1] + acc_arr[2] + acc_arr[3];
        acc_s *= alpha_t;
