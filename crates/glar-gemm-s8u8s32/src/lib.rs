...
pub(crate) unsafe fn glar_gemm_s8u8s32_fused<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
...
pub unsafe fn glar_gemm_s8u8s32(
    ...
    glar_gemm_s8u8s32_fused(m, n, k, alpha, a, b, beta, c, identity_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn glar_gemm_s8u8s32_fn_ptr(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    glar_gemm_s8u8s32_fused(m, n, k, alpha, a, b, beta, c, unary);
}
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glar_gemm_s8u8s32_fused(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_s8u8s32(
                                    m,
