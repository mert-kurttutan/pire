...
def_kernel_bb_v0!(i16, i16, i32, f32, f32, 8, 2, 8, 4);

use super::pack_sse::packa_panel_8;
use glar_base::def_kernel_sb_v0;
def_kernel_sb_v0!(i16, i16, i32, f32, f32, 2, 8, 2, 8, 4);
...
pub(crate) unsafe fn kernel<F: MyFn>(
    ...
    let k_eff = (k + 1) / 2 * 2;
    if c_rs == 1 {
        kernel_8x2_bb::<_, false>(m, n, k_eff, alpha, beta, c, c_rs, c_cs, ap, bp, f)
    } else {
        kernel_8x2_bb::<_, true>(m, n, k_eff, alpha, beta, c, c_rs, c_cs, ap, bp, f)
    }
}
