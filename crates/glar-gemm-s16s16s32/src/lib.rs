...
pub unsafe fn packa_i16_unchecked(
    m: usize,
    k: usize,
    a: *const TA,
    a_rs: usize,
...
pub unsafe fn packb_i16_unchecked(
    n: usize,
    k: usize,
    b: *const TB,
    b_rs: usize,
...
pub unsafe fn packa_i16(m: usize, k: usize, a: &[TA], a_rs: usize, a_cs: usize, ap: &mut [TA]) -> Array<TA> {
    ...
    unsafe { packa_i16_unchecked(m, k, a.as_ptr(), a_rs, a_cs, ap_ptr_o as *mut TA) }
}

pub unsafe fn packb_i16(n: usize, k: usize, b: &[TB], b_rs: usize, b_cs: usize, bp: &mut [TB]) -> Array<TB> {
    ...
    unsafe { packb_i16_unchecked(n, k, b.as_ptr(), b_rs, b_cs, bp_ptr_o as *mut TB) }
}
            ...
            for &n in &n_dims {
                for &k in &k_dims {
                    ...
                    let a_matrix = if is_a_packed {
                        unsafe { packa_i16(m, k, &a, a_rs, a_cs, &mut ap) }
                    } else {
                        Array::strided_matrix(a.as_ptr(), a_rs, a_cs)
                    };
                    let b_matrix = if is_b_packed {
                        unsafe { packb_i16(n, k, &b, b_rs, b_cs, &mut bp) }
                    } else {
