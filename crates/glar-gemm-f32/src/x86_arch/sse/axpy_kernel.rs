use std::{arch::x86::*, ptr::copy_nonoverlapping};
...
#[inline(always)]
unsafe fn v_loadu_n(mem_addr: *const TC, n: usize) -> __m128 {
    let mut a_arr = [0_f32; VS];
    copy_nonoverlapping(mem_addr, a_arr.as_mut_ptr(), n);
    _mm_loadu_ps(a_arr.as_ptr())
}

unsafe fn fmadd_ps(a: __m128, b: __m128, c: __m128) -> __m128 {
    let x = _mm_mul_ps(a, b);
    _mm_add_ps(x, c)
}
...
pub(crate) unsafe fn acc_store(
    ...
    let mut cv = cv;
    cv = fmadd_ps(_mm_loadu_ps(a.add(lda)), xt1, cv);
    cv = fmadd_ps(_mm_loadu_ps(a.add(lda * 2)), xt2, cv);
    cv = fmadd_ps(_mm_loadu_ps(a.add(lda * 3)), xt3, cv);
    _mm_storeu_ps(c, cv);
}
...
pub(crate) unsafe fn axpy_v_inner<const BETA: usize>(
    ...
    while mi < m_lane4 {
        seq!(i in 0..4 {
            let c~i = if BETA == 1 {
                fmadd_ps(_mm_loadu_ps(a.add(VS*i)), xt0, _mm_loadu_ps(y.add(VS*i)))
            } else if BETA == 0 {
                _mm_mul_ps(_mm_loadu_ps(a.add(VS*i)), xt0)
            } else {
                let cx~i = _mm_mul_ps(_mm_loadu_ps(y.add(VS*i)), beta_v);
                fmadd_ps(_mm_loadu_ps(a.add(VS*i)), xt0, cx~i)
            };
    ...
    while mi < m_lane {
        let c0 = if BETA == 1 {
            fmadd_ps(_mm_loadu_ps(a), xt0, _mm_loadu_ps(y))
        } else if BETA == 0 {
            _mm_mul_ps(_mm_loadu_ps(a), xt0)
        } else {
            let cx0 = _mm_mul_ps(_mm_loadu_ps(y), beta_v);
            fmadd_ps(_mm_loadu_ps(a), xt0, cx0)
        };
...
pub(crate) unsafe fn axpy_v(
    ...
    let mut a_cur = a;
    let mut x_cur = x;
    let mut xtv_arr = [_mm_setzero_ps(); K_UNROLL];
    while ni < n_lane {
        let mut xt_arr = [0.0; K_UNROLL];
        for i in 0..K_UNROLL {
            xt_arr[i] = *alpha * *x_cur.add(i * incx);
            xtv_arr[i] = _mm_set1_ps(xt_arr[i]);
        }
...
#[target_feature(enable = "sse")]
pub(crate) unsafe fn acc_vec(x: __m128) -> TC {
    let mut acc_arr = [0.0; VS];
    _mm_storeu_ps(acc_arr.as_mut_ptr(), x);
...
pub(crate) unsafe fn axpy_d(
    ...
    while i < m3 {
        ...
        let mut x_cur = x;
        let mut acc_arr = [_mm_setzero_ps(); 4 * 3];
        let mut p = 0;
        while p < n_iter_unroll_vec {
            seq!(q in 0..3 {
                acc_arr[q*4] = fmadd_ps(_mm_loadu_ps(a_cur.add(lda*q)), _mm_loadu_ps(x_cur), acc_arr[q*4]);
                acc_arr[q*4+1] = fmadd_ps(_mm_loadu_ps(a_cur.add(lda*q+VS)), _mm_loadu_ps(x_cur.add(VS)), acc_arr[q*4+1]);
                acc_arr[q*4+2] = fmadd_ps(_mm_loadu_ps(a_cur.add(lda*q+VS*2)), _mm_loadu_ps(x_cur.add(VS*2)), acc_arr[q*4+2]);
                acc_arr[q*4+3] = fmadd_ps(_mm_loadu_ps(a_cur.add(lda*q+VS*3)), _mm_loadu_ps(x_cur.add(VS*3)), acc_arr[q*4+3]);
            });
        ...
        seq!(q in 0..3 {
         ...
         acc_arr[q*4] = fmadd_ps(a_lef_v, x_left_v, acc_arr[q*4]);
         acc_arr[q*4] = _mm_add_ps(acc_arr[q*4], acc_arr[q*4+1]);
         acc_arr[q*4] = _mm_add_ps(acc_arr[q*4], acc_arr[q*4+2]);
         acc_arr[q*4] = _mm_add_ps(acc_arr[q*4], acc_arr[q*4+3]);
        });
    ...
    while i < m {
        let mut a_cur = a_cur0;
        let mut x_cur = x;
        let mut acc_arr = [_mm_setzero_ps(); 4];
        let mut p = 0;
        while p < n_iter_unroll_vec {
            seq!(q in 0..1 {
                acc_arr[q*4] = fmadd_ps(_mm_loadu_ps(a_cur.add(lda*q)), _mm_loadu_ps(x_cur), acc_arr[q*4]);
