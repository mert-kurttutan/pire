...
use pire_base::{
    get_cache_params, has_i16i32_compute, Array, ArrayMut, GemmCache, IdentityFn, PirePar, UnaryFn, AB_ALIGN,
};
...
pub(crate) unsafe fn pire_gemm_s16s16s32_fused<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
...
pub unsafe fn pire_gemm_s16s16s32(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
    ...
    pire_gemm_s16s16s32_fused(m, n, k, alpha, a, b, beta, c, identity_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn pire_gemm_s16s16s32_fn_ptr(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    pire_gemm_s16s16s32_fused(m, n, k, alpha, a, b, beta, c, unary);
}
...
mod tests {
    ...
    fn test_gemm(layout: &ABLayout, is_a_packed: bool, is_b_packed: bool) {
        ...
        let mut ap = vec![0i16; ap_size + AB_ALIGN];
        ...
        let mut bp = vec![0i16; bp_size + AB_ALIGN];
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                pire_gemm_s16s16s32_fused(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
