...
def_kernel_bb_v0_no_beta!(TA, TB, TC, TA, TC, 2, 2, 2);

use glar_base::def_kernel_bs_no_beta;

def_kernel_bs_no_beta!(TA, TB, TC, TA, TC, 2, 2, 2);

use super::pack_sse::packa_panel_2;

use glar_base::def_kernel_sb_v0_no_beta;

def_kernel_sb_v0_no_beta!(TA, TB, TC, TA, TC, 2, 2, 2);
...
pub(crate) unsafe fn kernel_sb<F: MyFn>(
    ...
    if c_rs == 1 {
        kernel_2x2_sb_v0::<_, false>(m, n, k, alpha, a, a_rs, a_cs, b, c, c_rs, c_cs, ap_buf, f);
    } else {
        kernel_2x2_sb_v0::<_, true>(m, n, k, alpha, a, a_rs, a_cs, b, c, c_rs, c_cs, ap_buf, f);
    }
}
...
pub(crate) unsafe fn kernel_bs<F: MyFn>(
    ...
    if c_rs == 1 {
        kernel_2x2_bs_v0::<_, false>(m, n, k, alpha, b, b_rs, b_cs, c, c_rs, c_cs, ap, f);
    } else {
        kernel_2x2_bs_v0::<_, true>(m, n, k, alpha, b, b_rs, b_cs, c, c_rs, c_cs, ap, f);
    }
}
...
pub(crate) unsafe fn kernel<F: MyFn>(
    ...
    if c_rs == 1 {
        kernel_2x2_bb::<_, false>(m, n, k, alpha, c, c_rs, c_cs, ap, bp, f)
    } else {
        kernel_2x2_bb::<_, true>(m, n, k, alpha, c, c_rs, c_cs, ap, bp, f)
    }
}
