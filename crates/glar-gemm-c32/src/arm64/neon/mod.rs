...
def_kernel_bb_v0!(TC, TC, TC, TC, TC, 12, 2, 12, 8, 4);
...
def_kernel_sb_v0!(TA, TB, TC, TA, TC, 1, 12, 2, 12, 8, 4);
...
pub(crate) unsafe fn kernel_sb<F: MyFn>(
    ...
    if c_rs == 1 {
        kernel_12x2_sb_v0::<_, false>(m, n, k, alpha, beta, a, a_rs, a_cs, b, c, c_rs, c_cs, ap_buf, f);
    } else {
        kernel_12x2_sb_v0::<_, true>(m, n, k, alpha, beta, a, a_rs, a_cs, b, c, c_rs, c_cs, ap_buf, f);
    }
}
...
pub(crate) unsafe fn kernel<F: MyFn>(
    ...
    if c_rs == 1 {
        kernel_12x2_bb::<_, false>(m, n, k, alpha, beta, c, c_rs, c_cs, ap, bp, f)
    } else {
        kernel_12x2_bb::<_, true>(m, n, k, alpha, beta, c, c_rs, c_cs, ap, bp, f)
    }
}
