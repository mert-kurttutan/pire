...
impl<F: MyFn> Arm64dispatcher<F> {
    ...
    pub(crate) unsafe fn packa_fn(&self, x: *const TA, y: *mut TA, m: usize, k: usize, rs: usize, cs: usize) {
        match self.reg_dim {
            RegDim::Reg12x2 => pack_neon::packa_panel_12(m, k, x, rs, cs, y, self.vs),
        }
    }

    pub(crate) unsafe fn packb_fn(&self, x: *const TB, y: *mut TB, n: usize, k: usize, rs: usize, cs: usize) {
        match self.reg_dim {
            RegDim::Reg12x2 => pack_neon::packb_panel_2(n, k, x, cs, rs, y),
        }
    }
...
unsafe fn glar_gemv<F: MyFn>(
    ...
    let incy = y.rs();
    match hw_cfg.reg_dim {
        RegDim::Reg12x2 => neon::axpy(m, n, alpha, a.src(), a.rs(), a.cs(), x_ptr, inc_x, beta, y_ptr, incy, hw_cfg.func),
    }
}
