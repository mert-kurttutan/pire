...
use glar_base::{
    ap_size, bp_size, get_cache_params, has_f32_compute, Array, ArrayMut, GemmCache, GlarPar, HWModel,
    RUNTIME_HW_CONFIG,
};
...
pub unsafe fn glar_cgemm(
    ...
    let null_fn = NullFn {};
    glar_cgemm_generic(m, n, k, alpha, a, b, beta, c, null_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn glar_cgemm_fused(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    glar_cgemm_generic(m, n, k, alpha, a, b, beta, c, unary);
}
...
pub unsafe fn glar_sgemv(
    ...
    glar_cgemm(m, 1, n, alpha, a, a_rs, a_cs, x, 1, incx, beta, y, 1, incy)
}
pub unsafe fn glar_sdot(
    ...
    glar_cgemm(1, 1, n, alpha, x, incx, 1, y, incy, 1, beta, res, 1, 1)
}
...
pub unsafe fn packa_c32(m: usize, k: usize, a: *const TA, a_rs: usize, a_cs: usize, ap: *mut TA) -> Array<TA> {
...
pub unsafe fn packb_c32(n: usize, k: usize, b: *const TB, b_rs: usize, b_cs: usize, bp: *mut TB) -> Array<TB> {
            ...
            for n in n_dims.iter() {
                ...
                for k in k_dims.iter() {
                    ...
                    let a_matrix = if is_a_packed {
                        unsafe { packa_c32_with_ref(m, k, &a, a_rs, a_cs, &mut ap) }
                    } else {
                    ...
                    let b_matrix = if is_b_packed {
                        unsafe { packb_c32_with_ref(n, k, &b, b_rs, b_cs, &mut bp) }
                    } else {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glar_cgemm_generic(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_c32(
                                    m,
