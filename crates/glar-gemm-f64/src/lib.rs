...
use glar_base::{
    get_cache_params, has_f64_compute, Array, ArrayMut, GemmCache, GlarPar, IdentityFn, UnaryFn, AB_ALIGN,
};
...
#[cfg(feature = "fuse")]
pub unsafe fn glar_dgemm_fused(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    glar_dgemm_generic(m, n, k, alpha, a, b, beta, c, unary);
}
...
pub unsafe fn packa_f64_unchecked(
    m: usize,
    k: usize,
    a: *const TA,
    a_rs: usize,
...
pub unsafe fn packb_f64_unchecked(
    n: usize,
    k: usize,
    b: *const TB,
    b_rs: usize,
...
pub unsafe fn packa_f64(m: usize, k: usize, a: &[TA], a_rs: usize, a_cs: usize, ap: &mut [TA]) -> Array<TA> {
    ...
    unsafe { packa_f64_unchecked(m, k, a.as_ptr(), a_rs, a_cs, ap_ptr_o as *mut TA) }
}

pub unsafe fn packb_f64(n: usize, k: usize, b: &[TB], b_rs: usize, b_cs: usize, bp: &mut [TB]) -> Array<TB> {
    ...
    unsafe { packb_f64_unchecked(n, k, b.as_ptr(), b_rs, b_cs, bp_ptr_o as *mut TB) }
}
            ...
            for &n in &n_dims {
                for &k in &k_dims {
                    ...
                    let a_matrix = if is_a_packed {
                        unsafe { packa_f64(m, k, &a, a_rs, a_cs, &mut ap) }
                    } else {
                        Array::strided_matrix(a.as_ptr(), a_rs, a_cs)
                    };
                    let b_matrix = if is_b_packed {
                        unsafe { packb_f64(n, k, &b, b_rs, b_cs, &mut bp) }
                    } else {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glar_dgemm_generic(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_f64(
                                    m,
