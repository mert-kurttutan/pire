...
pub(crate) unsafe fn glar_dgemm_fused<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
...
pub unsafe fn glar_dgemm(
    ...
    glar_dgemm_fused(m, n, k, alpha, a, b, beta, c, identity_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn glar_dgemm_fn_ptr(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    glar_dgemm_fused(m, n, k, alpha, a, b, beta, c, unary);
}
    ...
    fn test_pack_a() {
        ...
        for &m in &m_dims {
            for &k in &k_dims {
                ...
                let mut ap = vec![0.0; a_size + AB_ALIGN];
    ...
    fn test_pack_b() {
        ...
        for &n in &n_dims {
            for &k in &k_dims {
                ...
                let mut bp = vec![0.0; b_size + AB_ALIGN];
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glar_dgemm_fused(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_f64(
                                    m,
