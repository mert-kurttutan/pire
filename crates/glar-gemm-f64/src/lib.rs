...
use glar_base::{
    ap_size, bp_size, get_cache_params, has_f64_compute, Array, ArrayMut, GemmCache, GlarPar, HWModel, StridedMatrix,
...
pub unsafe fn glar_dgemv(
    ...
    glar_dgemm(m, 1, n, alpha, a, a_rs, a_cs, x, 1, incx, beta, y, 1, incy)
}
pub unsafe fn glar_sdot(
    ...
    glar_dgemm(1, 1, n, alpha, x, incx, 1, y, incy, 1, beta, res, 1, 1)
}
...
pub unsafe fn packb_f64_with_ref(n: usize, k: usize, b: &[TB], b_rs: usize, b_cs: usize, bp: &mut [TB]) -> Array<TB> {
    ...
    unsafe { packb_f64(n, k, b.as_ptr(), b_rs, b_cs, bp.as_mut_ptr()) }
}
        ...
        for m in m_dims.iter() {
            ...
            for n in n_dims.iter() {
                ...
                for k in k_dims.iter() {
                    ...
                    let mut ap = vec![0_f64; ap_size];
                    let a_matrix = if is_a_packed {
                        unsafe { packa_f64_with_ref(m, k, &a, a_rs, a_cs, &mut ap) }
                    } else {
                    ...
                    let b_matrix = if is_b_packed {
                        unsafe { packb_f64_with_ref(n, k, &b, b_rs, b_cs, &mut bp) }
                    } else {
