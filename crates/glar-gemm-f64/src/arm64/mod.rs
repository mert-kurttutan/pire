...
impl<F: MyFn> Arm64dispatcher<F> {
    pub(crate) fn from_hw_cfg(hw_config: &HWConfig, f: F) -> Self {
        ...
        let (mr, nr, reg_dim) = (12, 4, RegDim::Reg12x4);
        let vs = 4;
        Self {
            mc,
            nc,
    ...
    pub(crate) unsafe fn packb_fn(&self, x: *const TB, y: *mut TB, n: usize, k: usize, rs: usize, cs: usize) {
        match self.reg_dim {
            RegDim::Reg12x4 => pack_neon::packb_panel_4(n, k, x, cs, rs, y),
        }
    }
...
impl<T: MyFn> GemmCache for Arm64dispatcher<T> {
    fn mr(&self) -> usize {
        self.mr
    }
    fn nr(&self) -> usize {
...
unsafe fn kernel<F: MyFn>(
    ...
    if kc_last {
        match hw_cfg.reg_dim {
            RegDim::Reg12x4 => neon::kernel(m, n, k, alpha, beta, c, c_rs, c_cs, ap, bp, hw_cfg.func),
        }
    } else {
        let null_fn = NullFn {};
        match hw_cfg.reg_dim {
            RegDim::Reg12x4 => neon::kernel(m, n, k, alpha, beta, c, c_rs, c_cs, ap, bp, null_fn),
        }
    }
}
...
unsafe fn glar_gemv<F: MyFn>(
    ...
    let incy = y.rs();
    match hw_cfg.reg_dim {
        RegDim::Reg12x4 => neon::axpy(m, n, alpha, a.src(), a.rs(), a.cs(), x_ptr, inc_x, beta, y_ptr, incy, hw_cfg.func),
    }
}
