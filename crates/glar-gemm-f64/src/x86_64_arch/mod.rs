...
#[inline(always)]
pub fn get_mcnckc_simd() -> (usize, usize, usize) {
...
pub(crate) unsafe fn packa_fn_simd(x: *const TA, y: *mut TA, m: usize, k: usize, rs: usize, cs: usize) {
    ...
    if hw_config.cpu_ft.avx512f {
        ...
        pack_avx::packa_panel_8(m, k, x, rs, cs, y, AVX_VS);
    } else {
        pack_sse::packa_panel_4(m, k, x, rs, cs, y, SSE_VS);
    }
}
...
impl<F: UnaryFnC> KernelDispatcher<F> {
    pub(crate) fn new(f: F) -> Self {
        ...
        let (mr, nr, reg_dim) = if features.avx512f {
            (AVX512F_MR, AVX512F_NR, RegDim::Reg24x8)
        } else if features.avx && features.fma {
            (AVXFMA_MR, AVXFMA_NR, RegDim::Reg12x4)
        } else if features.avx {
            (AVX_MR, AVX_NR, RegDim::Reg8x4)
        } else {
            (SSE_MR, SSE_NR, RegDim::Reg4x4)
        };
...
unsafe fn kernel_m<F: UnaryFnC>(
    ...
    if kc_last {
        match hw_cfg.reg_dim {
            RegDim::Reg24x8 => avx512f::kernel_bs(m, n, k, alpha, beta, b, b_rs, b_cs, c, c_rs, c_cs, ap, hw_cfg.func),
            RegDim::Reg12x4 => avx_fma::kernel_bs(m, n, k, alpha, beta, b, b_rs, b_cs, c, c_rs, c_cs, ap, hw_cfg.func),
        ...
        match hw_cfg.reg_dim {
            ...
            RegDim::Reg12x4 => avx_fma::kernel_bs(m, n, k, alpha, beta, b, b_rs, b_cs, c, c_rs, c_cs, ap, null_fn),
...
unsafe fn kernel_n<F: UnaryFnC>(
    ...
    if kc_last {
        ...
        let null_fn = IdentityFn {};
        match hw_cfg.reg_dim {
            RegDim::Reg24x8 => avx512f::kernel_sb(m, n, k, alpha, beta, a, a_rs, a_cs, b, c, c_rs, c_cs, ap, null_fn),
            RegDim::Reg12x4 => avx_fma::kernel_sb(m, n, k, alpha, beta, a, a_rs, a_cs, b, c, c_rs, c_cs, ap, null_fn),
