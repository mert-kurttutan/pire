...
pub(crate) unsafe fn acc_store(
    ...
    let mut cv = cv;
    cv = fmadd_ps(v_load_ph(a.add(lda)), xt1, cv);
    cv = fmadd_ps(v_load_ph(a.add(lda * 2)), xt2, cv);
    cv = fmadd_ps(v_load_ph(a.add(lda * 3)), xt3, cv);
...
pub(crate) unsafe fn axpy_v_inner<const BETA: usize>(
    ...
    while mi < m_lane4 {
        seq!(i in 0..4 {
            let c~i = if BETA == 1 {
                fmadd_ps(v_load_ph(a.add(VS*i)), xt0, v_load_ph(y.add(VS*i)))
            } else if BETA == 0 {
                _mm256_mul_ps(v_load_ph(a.add(VS*i)), xt0)
            } else {
                let cx~i = _mm256_mul_ps(v_load_ph(y.add(VS*i)), beta_v);
                fmadd_ps(v_load_ph(a.add(VS*i)), xt0, cx~i)
            };
...
pub(crate) unsafe fn axpy_d(
    ...
    while i < m3 {
        ...
        while p < n_iter_unroll_vec {
            seq!(q in 0..3 {
                acc_arr[q*4] = fmadd_ps(v_load_ph(a_cur.add(lda*q)), v_load_ph(x_cur), acc_arr[q*4]);
                acc_arr[q*4+1] = fmadd_ps(v_load_ph(a_cur.add(lda*q+VS)), v_load_ph(x_cur.add(VS)), acc_arr[q*4+1]);
                acc_arr[q*4+2] = fmadd_ps(v_load_ph(a_cur.add(lda*q+VS*2)), v_load_ph(x_cur.add(VS*2)), acc_arr[q*4+2]);
                acc_arr[q*4+3] = fmadd_ps(v_load_ph(a_cur.add(lda*q+VS*3)), v_load_ph(x_cur.add(VS*3)), acc_arr[q*4+3]);
            });
