...
pub(crate) unsafe fn pire_hgemm_fused<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
    ...
    if has_f16f32_compute() {
        #[cfg(target_arch = "x86_64")]
        {
            ...
            pire_gemm_f32(&hw_config, m, n, k, alpha.to_f32(), a, b, beta.to_f32(), c, &par);
            return;
        }
    }
    ...
    reference::pire_gemm(&hw_config, m, n, k, alpha.to_f32(), a, b, beta.to_f32(), c, &par);
}

pub unsafe fn pire_hgemm(
    m: usize,
    n: usize,
    k: usize,
    alpha: f16,
    ...
    pire_hgemm_fused(m, n, k, alpha, a, b, beta, c, identity_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn pire_hgemm_fn_ptr(
    m: usize,
    n: usize,
    k: usize,
    alpha: f16,
    ...
    fn test_pack_a() {
        ...
        for &m in &m_dims {
            for &k in &k_dims {
                ...
                let a = vec![TA::ZERO; m * k * a_stride_scale];
                let mut ap = vec![TA::ZERO; a_size + AB_ALIGN];
    ...
    fn test_pack_b() {
        ...
        for &n in &n_dims {
            for &k in &k_dims {
                ...
                let b = vec![TB::ZERO; n * k * b_stride_scale];
                let mut bp = vec![TB::ZERO; b_size + AB_ALIGN];
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                pire_hgemm_fused(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_f16(
                                    m,
