...
#[cfg(target_arch = "x86_64")]
use x86_64_arch::{get_mcnckc_simd, packa_fn_simd, packb_fn_simd, pire_gemm, round_mnk_simd, KernelDispatcher};
...
use pire_base::{
    get_cache_params, has_i8i32_compute, Array, ArrayMut, GemmCache, IdentityFn, PirePar, UnaryFn, AB_ALIGN,
};

pub trait UnaryFnC: UnaryFn<TC> {}
impl<F: UnaryFn<TC>> UnaryFnC for F {}

pub(crate) unsafe fn pire_gemm_s8u8s32_fused<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
...
pub unsafe fn pire_gemm_s8u8s32(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
    ...
    pire_gemm_s8u8s32_fused(m, n, k, alpha, a, b, beta, c, identity_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn pire_gemm_s8u8s32_fn_ptr(
    m: usize,
    n: usize,
    k: usize,
    alpha: f32,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    pire_gemm_s8u8s32_fused(m, n, k, alpha, a, b, beta, c, unary);
}
...
mod tests {
    ...
    fn test_pack_a() {
        ...
        for &m in &m_dims {
            for &k in &k_dims {
                ...
                let a = vec![0i8; m * k * a_stride_scale];
                let mut ap = avec![[AB_ALIGN]| 0i8; a_size];
    ...
    fn test_pack_b() {
        ...
        for &n in &n_dims {
            for &k in &k_dims {
                ...
                let b = vec![0u8; n * k * b_stride_scale];
                let mut bp = avec![[AB_ALIGN]| 0u8; b_size];
    ...
    fn test_gemm(layout: &ABLayout, is_a_packed: bool, is_b_packed: bool) {
        ...
        let mut ap = vec![0i8; ap_size + AB_ALIGN];
        ...
        let mut bp = vec![0u8; bp_size + AB_ALIGN];
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                pire_gemm_s8u8s32_fused(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_s8u8s32(
                                    m,
