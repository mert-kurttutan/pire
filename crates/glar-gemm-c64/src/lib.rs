...
use glar_base::{
    ap_size, bp_size, has_c64_compute, Array, ArrayMut, GemmCache, GlarPar, IdentityFn, UnaryFn, RUNTIME_HW_CONFIG,
};

pub(crate) trait UnaryFnC: UnaryFn<TC> {}
impl<F: UnaryFn<TC>> UnaryFnC for F {}

pub(crate) unsafe fn glar_zgemm_generic<F: UnaryFnC>(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
...
pub unsafe fn glar_zgemm(
    ...
    glar_zgemm_generic(m, n, k, alpha, a, b, beta, c, null_fn);
}

#[cfg(feature = "fuse")]
pub unsafe fn glar_zgemm_fused(
    m: usize,
    n: usize,
    k: usize,
    alpha: TA,
    ...
    let b = Array::strided_matrix(b, b_rs, b_cs);
    let c = ArrayMut::strided_matrix(c, c_rs, c_cs);
    glar_zgemm_generic(m, n, k, alpha, a, b, beta, c, unary);
}
...
pub unsafe fn packa_c64(m: usize, k: usize, a: *const TA, a_rs: usize, a_cs: usize, ap: *mut TA) -> Array<TA> {
...
pub unsafe fn packb_c64(n: usize, k: usize, b: *const TB, b_rs: usize, b_cs: usize, bp: *mut TB) -> Array<TB> {
...
pub unsafe fn packa_c64_with_ref(m: usize, k: usize, a: &[TA], a_rs: usize, a_cs: usize, ap: &mut [TA]) -> Array<TA> {
    let pack_size = ap_size::<TA>(m, k);
    let ap_ptr = ap.as_mut_ptr() as *mut f64;
    let ap_align_offset = ap_ptr.align_offset(glar_base::AB_ALIGN);
    // safety check
    assert!(ap.len() >= pack_size);
    let ap_ptr_o = ap_ptr.add(ap_align_offset);
    unsafe { packa_c64(m, k, a.as_ptr(), a_rs, a_cs, ap_ptr_o as *mut TA) }
}

pub unsafe fn packb_c64_with_ref(n: usize, k: usize, b: &[TB], b_rs: usize, b_cs: usize, bp: &mut [TB]) -> Array<TB> {
    let pack_size = bp_size::<TB>(n, k);
    let bp_ptr = bp.as_mut_ptr() as *mut f64;
    let bp_align_offset = bp_ptr.align_offset(glar_base::AB_ALIGN);
    // safety check
    assert!(bp.len() >= pack_size);
    let bp_ptr_o = bp_ptr.add(bp_align_offset);
    unsafe { packb_c64(n, k, b.as_ptr(), b_rs, b_cs, bp_ptr_o as *mut TB) }
}
...
mod tests {
    ...
    use glar_dev::{
        check_gemm_c64, generate_k_dims, generate_m_dims, generate_n_dims, layout_to_strides, random_matrix_uniform,
        ABLayout,
    };
                ...
                for &k in &k_dims {
                    ...
                    for alpha in ALPHA_ARR {
                        for beta in BETA_ARR {
                            ...
                            unsafe {
                                glar_zgemm_generic(m, n, k, alpha, a_matrix, b_matrix, beta, c_matrix, unary_fn);
                            }
                            let diff_max = unsafe {
                                check_gemm_c64(
                                    m,
