...
use glare_base::{
    ap_size_int, bp_size_int, get_cache_params, has_i8i32_compute, Array, ArrayMut, GemmCache, GlarePar, HWModel,
...
fn get_mcnckc() -> (usize, usize, usize) {
    // let mc = std::env::var("GLARE_MC").unwrap_or("5400".to_string()).parse::<usize>().unwrap();
    ...
    let (mc, nc, kc) = match (*RUNTIME_HW_CONFIG).hw_model {
        ...
        HWModel::Haswell | HWModel::Broadwell => (4800, 320, 768),
        _ => get_cache_params(),
    };
    (mc, nc, kc)
}
